...
pub async fn market_order(postgres_client: &Client, req: MarketOrderRequest) -> MarketOrderResponse {
    let pg_side = req.side as i16;
    let pg_leverage = req.leverage as i16;
    let user_id: i32 = match req.user_id.parse() {
        Ok(id) => id,
        Err(_) => return MarketOrderResponse { success: false },
    };
    ...
    match result {
        Ok(_) => {
            println!("\n> [MARKET_ORDER_DB]: order_id:{}, symbol:{}, quantity:{}, side:{}, type:{}, leverage:{}, tp:{}, sl:{}, open:{}",
                req.order_id, req.symbol, req.quantity, req.side, req.order_type, req.leverage, req.tp, req.sl, req.open);
            MarketOrderResponse { success: true }
        }
        Err(err) => {
            log_db_error("market_order", &err);
            MarketOrderResponse { success: false }
        }
    }
}